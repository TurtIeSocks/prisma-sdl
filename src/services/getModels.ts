...
import type { Extension, Model, ModelTemplate, Schema, ValidType } from '../assets/types'
...
export function getModels(schema: Schema, singleMode: boolean, fileTypes: Extension[]): ModelTemplate[] {
  const models = (schema.schema.match(/.+{([^}]*)}/g) || [])
    .filter((model) => model.startsWith('model') && !model.split(' ')[1]?.endsWith('s'))
    ...
    .map((model) => {
      ...
      const cleanModel: Model = {
        ...
        properties: rest
          .map((prop) => {
            const [propName, propType, details] = prop.split(' ').filter((word) => word.trim())
            if (propName && propType) {
              const jsonCheck = propType.includes('Json') ? 'JSON' : propType
              return {
                name: propName,
                type: (VALID_TYPES.includes(jsonCheck as typeof VALID_TYPES[number]) ? jsonCheck.replace(/\?/g, '') : '') as ValidType,
                optional: propType.includes('?'),
                auto: details ? details.includes('now()') || details.includes('@updatedAt') : false,
              }
            }
            return {
              name: '',
      ...
      return {
        ...
        templates: {
          serverQueryAll: {
            ...
            ...Object.fromEntries(fileTypes.map((type) => [type, queryAll(schema, cleanModel, singleMode, type)])),
          },
          serverQueryOne: {
            fileName: cleanModel.camel,
            location: 'server/resolvers/queryOne',
            ...Object.fromEntries(fileTypes.map((type) => [type, queryOne(schema, cleanModel, singleMode, type)])),
          },
          serverMut: {
            fileName: `edit${cleanModel.pascalPlural}`,
            location: 'server/resolvers/mutations',
            ...Object.fromEntries(fileTypes.map((type) => [type, mutation(schema, cleanModel, singleMode, type)])),
          },
          serverTdQueries: {
            fileName: cleanModel.screamingSnake,
            location: 'server/typeDefs/queries',
            ...Object.fromEntries(fileTypes.map((type) => [type, serverSDL(cleanModel, false, type)])),
          },
          serverTdMut: {
            fileName: `${cleanModel.screamingSnake}_INPUT`,
            location: 'server/typeDefs/mutations',
            ...Object.fromEntries(fileTypes.map((type) => [type, serverSDL(cleanModel, true, type)])),
          },
          clientAllQuery: {
            fileName: cleanModel.screamingSnakePlural,
            location: 'client/queryAll',
            ...Object.fromEntries(fileTypes.map((type) => [type, clientSDL(cleanModel, false, true, type)])),
          },
          clientOneQuery: {
            fileName: cleanModel.screamingSnake,
            location: 'client/queryOne',
            ...Object.fromEntries(fileTypes.map((type) => [type, clientSDL(cleanModel, false, false, type)])),
          },
          clientMut: {
            fileName: `EDIT_${cleanModel.screamingSnakePlural}`,
            location: 'client/mutations',
            ...Object.fromEntries(fileTypes.map((type) => [type, clientSDL(cleanModel, true, false, type)])),
          },
          hookAll: {
            fileName: `use${cleanModel.pascalPlural}`,
            location: 'client/hooks/queryAll',
            ...Object.fromEntries(fileTypes.map((type) => [type, hookAll(cleanModel, type)])),
          },
          hookOne: {
            fileName: `use${cleanModel.pascal}`,
            location: 'client/hooks/queryOne',
            ...Object.fromEntries(fileTypes.map((type) => [type, hookOne(cleanModel, type)])),
          },
          hookMut: {
            fileName: `useEdit${cleanModel.pascalPlural}`,
            location: 'client/hooks/mutations',
            ...Object.fromEntries(fileTypes.map((type) => [type, hookMut(cleanModel, type)])),
          },
          tsTypes: {
            fileName: cleanModel.camelPlural,
            location: 'types',
            ...Object.fromEntries(fileTypes.map((type) => [type, clientQueries(schema, cleanModel, singleMode, type)])),
          },
        },
      }
    })
