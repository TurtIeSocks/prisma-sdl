...
export function prismaSdl({
  ...
  const generatedSchemas: ReturnObj[] = schemas.map((schema) => {
    const models = getModels(schema, singleMode, internalFileTypes, customTemplates)
    return {
      models,
      schema,
      resolvers: Object.fromEntries(
        internalFileTypes.map((ext) => [
          ext,
          templater(customTemplates?.allResolvers?.[ext] || allResolvers(ext), schema, singleMode, models),
        ]),
      ),
      typeDefs: Object.fromEntries(
        internalFileTypes.map((ext) => [
          ext,
          templater(customTemplates?.typeDefs?.[ext] || allTypeDefs(ext), schema, singleMode, models),
        ]),
      ),
      context: Object.fromEntries(
        internalFileTypes.map((ext) => [
          ext,
          templater(customTemplates?.context?.[ext] || gqlContext(ext), schema, singleMode, models),
        ]),
      ),
    }
  })
